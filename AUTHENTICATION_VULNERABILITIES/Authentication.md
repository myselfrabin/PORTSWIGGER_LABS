...
# VULNERABILITIES IN OTHER LOGIN MECHANISH:    
1. Basic login page me the website provide the supplementarty functionality to the user to manage their account. Example: User can typically change their password or reset their password when they forget it. This mechanism does introduced vulnerabilities that can be exploited by an attacker.
2. Modern website usually take care to avoid bugs in the login page, but it's easy to know the fact that we need to take similar steps to exploit the vulnerabilities when we does found.
3. Attacker can create their own account and have access to the additional pages in the website.

# KEEPING USER LOGGED IN:
1. A common feature in modern day website is option to keep user loggedin even after closing the browser session. This is usually simple checkbox something like: **Remember Me** or **Keep me logged in**.
2. This feature is usually implemented using **cookies** or **session**.
3. The funconality is often generated by **Remember Me** token of some kind, which is then stored in a persistent cookie. As playing with this cookie effectively allow us to bypass entire login process.
4. Some website assume if the cookie is encrypted in some way it will not be guessable even if does use the static values. This may be true if done correctly 
